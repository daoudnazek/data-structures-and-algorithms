...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach(element => {
    let clonedItem = $('#template').clone();
    clonedItem.removeAttr('id');
    clonedItem.find('h2').text(element.name);
    clonedItem.find('h3').text(element.height);
    clonedItem.find('p').text(element.eye_color);
    $('main').append(clonedItem);
  })
...
const howMuchPencil = (str) => {
  ...
  for (let i=0; i<str.length+1; i++){
    ...
    let strJoint = strSplit.slice(i).join('');
    result.push(strJoint);
  }
  return result;
};
...
const listFoods = (recipe) => {
  ...
  let firstSlicedArr = splitArr.slice(splitArr.indexOf(' ')+1);
  let secondSlicedArr = firstSlicedArr.slice(firstSlicedArr.indexOf(' ')+1);
  result.push(secondSlicedArr.join(''));
  })
  return result;
};
