...
const createServer = () => {
  ...
  app.get('/aboutme', function (req, res) {
    res.status(200).send('My name is Daoud');
  });

  app.get('/favoritefoods', function (req, res) {
    res.status(200).send(['Pasta', 'Burger']);
  });
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
 let arr = input.map(e =>{
   return e.filter(a => !(a%5) && !(typeof a === 'string'))
 });

 let result = arr.map(e => {
   return e.map(a=> Math.pow(2,a))
 });
 return result;
};
